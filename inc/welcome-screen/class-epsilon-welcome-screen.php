...
class Epsilon_Welcome_Screen {
	...
	public function __construct( $config = array() ) {
		...
		$defaults = array(
			...
			'author-logo' => get_template_directory_uri() . '/inc/welcome-screen/img/colorlib.png',
			'actions'     => array(),
			'plugins'     => array(),
			'notice'      => '',
			'sections'    => array(),
		...
		if ( isset( $_POST['args'], $_POST['args']['nonce'] ) && ! wp_verify_nonce( sanitize_key( $_POST['args']['nonce'] ), 'welcome_nonce' ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Not allowed', 'datarc' ),
					)
				)
			);
		}
		...
		if ( ! class_exists( $args_action[0] ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Class does not exist', 'datarc' ),
					)
				)
			);
		}
	...
	public function enqueue() {
		if ( is_admin() ) {
			wp_enqueue_style(
				'welcome-screen',
				get_template_directory_uri() . '/inc/welcome-screen/css/welcome.css'
			);

			wp_enqueue_script(
				'welcome-screen',
				get_template_directory_uri() . '/inc/welcome-screen/js/welcome.js',
				array(
					'jquery-ui-slider',
				),
				'12123'
			...
			wp_localize_script(
				...
				array(
					...
					'no_required_actions_text' => esc_html__( 'Hooray! There are no required actions for you right now.', 'datarc' ),
					'ajax_nonce'               => wp_create_nonce( 'welcome_nonce' ),
					'activating_string'        => esc_html__( 'Activating', 'datarc' ),
					'body_class'               => 'appearance_page_' . $this->theme_slug . '-welcome',
					'no_actions'               => esc_html__( 'Hooray! There are no required actions for you right now.', 'datarc' ),
				)
			);
		}
	}
	...
	private function add_admin_notice() {
		...
		if ( empty( $this->notice ) ) {
			...
			$this->notice .= '<h1>' . sprintf( esc_html__( 'Welcome to %1$s', 'datarc' ), $this->theme_name ) . '</h1>';
			$this->notice .= '<p>';
			$this->notice .=
			...
			$this->notice .= '<p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome&tab=recommended-actions' ) ) . '" class="button button-primary button-hero" style="text-decoration: none;"> ' . sprintf( esc_html__( 'Get started with %1$s', 'datarc' ), $this->theme_name ) . '</a></p>';
		...
		$notifications->add_notice(
			array(
				'id'      => 'datarc_welcome_notification',
	...
	public function welcome_screen_menu() {
		...
		$title = sprintf( esc_html__( 'About %1$s', 'datarc' ), esc_html( $this->theme_name ) );
	...
	private function set_default_sections( $config = array() ) {
		$arr = array(
			'getting-started'     => array(
				...
				'label' => __( 'Getting Started', 'datarc' ),
				'path'  => get_template_directory() . '/inc/welcome-screen/sections/getting-started.php',
			),
			'recommended-actions' => array(
				'id'    => 'recommended-actions',
				'url'   => $this->generate_admin_url( 'recommended-actions' ),
				'label' => __( 'Recommended Actions', 'datarc' ),
				'path'  => get_template_directory() . '/inc/welcome-screen/sections/recommended-actions.php',
			),
			'recommended-plugins' => array(
				'id'    => 'recommended-plugins',
				'url'   => $this->generate_admin_url( 'recommended-plugins' ),
				'label' => __( 'Recommended Plugins', 'datarc' ),
				'path'  => get_template_directory() . '/inc/welcome-screen/sections/recommended-plugins.php',
			),
			'support'             => array(
				'id'    => 'support',
				'url'   => $this->generate_admin_url( 'support' ),
				'label' => __( 'Support', 'datarc' ),
				'path'  => get_template_directory() . '/inc/welcome-screen/sections/support.php',
			),
			'registration'        => array(
				'id'    => 'registration',
				'url'   => $this->generate_admin_url( 'registration' ),
				'label' => __( 'Registration', 'datarc' ),
				'path'  => get_template_directory() . '/inc/welcome-screen/sections/registration.php',
			),
		);
	...
	private function check_plugin( $slug = '' ) {
		$arr = array(
			...
			'label'     => __( 'Install and Activate', 'datarc' ),
		);
		...
		if ( $arr['installed'] ) {
			...
			$arr['label'] = __( 'Activate now', 'datarc' );
		}

		if ( $arr['active'] ) {
			$arr['needs'] = 'deactivate';
			$arr['class'] = 'deactivate-now button button-primary';
			$arr['label'] = __( 'Deactivate now', 'datarc' );
		}
