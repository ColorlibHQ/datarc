    ...
    <?php endif; ?>
        <div class="comment--meta-info">
            ...
            <div class="comment__info">
                <cite><?php printf( __( '<span class="comment-author-name">%s</span> ', 'datarc' ), get_comment_author_link() ); ?></cite>
                <div class="comment__meta">
                    <time class="comment__time"> <?php printf( __('%1$s at %2$s', 'datarc'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'datarc' ), '  ', '' ); ?></time> 
...
// social media
if ( ! function_exists( 'datarc_social' ) ) {
	function datarc_social( $args = array()  ){
		...
		$url = datarc_opt('datarc_social_url');
...
// header cart count
function datarc_cart_count( $class= '' ){
	
	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( DATARC_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_html_e( 'ICON', 'datarc' ); ?>">
