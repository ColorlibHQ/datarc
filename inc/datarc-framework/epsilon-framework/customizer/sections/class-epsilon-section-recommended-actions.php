...
class Epsilon_Section_Recommended_Actions extends WP_Customize_Section {
	...
	public function json() {
		...
		$json['facebook_text']  = esc_html__( 'Facebook', 'datarc' );
		$json['twitter']        = $this->twitter;
		$json['twitter_text']   = esc_html__( 'Twitter', 'datarc' );
		$json['wp_review']      = $this->wp_review;
		$json['wp_review_text'] = esc_html__( 'Review this theme on w.org', 'datarc' );
		if ( $this->wp_review ) {
			...
			if ( ! empty( $v['plugin_slug'] ) ) {
				...
				switch ( $active['needs'] ) {
					case 'install':
						...
						$v['button_label'] = esc_html__( 'Install', 'datarc' );
						break;
					case 'activate':
						$v['button_class'] = 'activate-now button button-primary';
						$v['button_label'] = esc_html__( 'Activate', 'datarc' );
						break;
					case 'update':
						$v['button_class'] = 'update-now button button-primary';
						$v['button_label'] = esc_html__( 'Update', 'datarc' );
						break;
					case 'deactivate':
						$v['button_class'] = 'deactivate-now button';
						$v['button_label'] = esc_html__( 'Deactivate', 'datarc' );
						break;
				}
		...
		foreach ( $this->plugins as $k => $v ) {
			...
			switch ( $active['needs'] ) {
				case 'install':
					...
					$t['button_label'] = esc_html__( 'Install', 'datarc' );
					break;
				case 'activate':
					$t['button_class'] = 'activate-now button button-primary';
					$t['button_label'] = esc_html__( 'Activate', 'datarc' );
					break;
				case 'update':
					$t['button_class'] = 'update-now button button-primary';
					$t['button_label'] = esc_html__( 'Update', 'datarc' );
					break;
				case 'deactivate':
					$t['button_class'] = 'deactivate-now button';
					$t['button_label'] = esc_html__( 'Deactivate', 'datarc' );
					break;
			}
