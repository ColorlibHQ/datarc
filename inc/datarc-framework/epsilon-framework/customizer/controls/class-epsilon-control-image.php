		...
		<div class="epsilon-controller-image-container image-upload">
			...
			<div class="placeholder">
				...
				<# if ( ! _.isUndefined( data.sizeArray[data.size] ) ) { #>
					<span class="recommended-size"><?php echo esc_html__('Recommended resolution:', 'datarc'); ?> {{{ data.sizeArray[data.size].width }}} x {{{ data.sizeArray[data.size].height }}}</span>
				<# } #>
			</div>
			<# } #>
			<div class="actions">
				...
				<button type="button" class="button-secondary image-default-button" <# if ( _.isEmpty( data.default ) ) { #> style="display:none;" <# } #>>
					<?php echo esc_html__( 'Default', 'datarc' ); ?>
				</button>
