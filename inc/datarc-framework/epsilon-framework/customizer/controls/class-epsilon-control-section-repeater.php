...
class Epsilon_Control_Section_Repeater extends WP_Customize_Control {
	...
	public function set_repeatable_sections() {
		...
		foreach ( $this->repeatable_sections as $key => $value ) {
			...
			$this->repeatable_sections[ $key ]['fields'][ $key . '_section_class' ] = array(
				'label'   => esc_html__( 'Section Class', 'datarc' ),
			...
			foreach ( $value['fields'] as $k => $v ) {
				...
				if ( 'epsilon-upsell' === $v['type'] ) {
					$this->repeatable_sections[ $key ]['fields'][ $k ]['label']              = ! empty( $this->repeatable_sections[ $key ]['fields'][ $k ]['label'] ) ? $this->repeatable_sections[ $key ]['fields'][ $k ]['label'] : __( 'See what\'s in the PRO version', 'datarc' );
	...
	public function content_template() {
		...
		<p class="limit"><?php echo esc_html__( 'Limit: ', 'datarc' ); ?>
			{{{ data.choices.limit }}} <?php echo esc_html__( 'sections', 'datarc' ); ?></p>
		<# } #>
		<div class="epsilon-add-section-buttons">
			...
			<button type="button" class="button epsilon-add-new-section" aria-expanded="false" aria-controls="available-sections">
				<?php esc_html_e( 'Add a Section', 'datarc' ); ?>
			</button>
		</div>
		<div id="sections-left-{{ data.id }}">
			<div class="available-sections">
				<div class="available-sections-filter">
					<label class="screen-reader-text" for="sections-search-{{ data.id }}"><?php esc_html_e( 'Search sections', 'datarc' ); ?></label>
					<input type="text" class="sections-search-input" id="sections-search-{{ data.id }}" placeholder="<?php esc_attr_e( 'Search sections &hellip;', 'datarc' ) ?>" aria-describedby="sections-search-desc"/>
					<div class="search-icon" aria-hidden="true"></div>
					<button type="button" class="clear-results">
						<span class="screen-reader-text"><?php esc_html_e( 'Clear Results', 'datarc' ); ?></span>
					</button>
					<p class="screen-reader-text" id="sections-search-desc-{{ data.id }}"><?php esc_html_e( 'The search results will be updated as you type.', 'datarc' ); ?></p>
				</div>
				<div class="available-sections-list">
					<# if ( data.integrations ) { #>
						<nav class="available-sections-tab-nav">
							<a href="#" data-tab="normal" class="available-sections-tab-toggler active"><span class="dashicons dashicons-menu"></span> <?php esc_html_e( 'Sections', 'datarc' ); ?></a>
							<a href="#" data-tab="integrations" class="available-sections-tab-toggler"><span class="dashicons dashicons-admin-plugins"></span> <?php esc_html_e( 'Integrations', 'datarc' ); ?> <span class="badge">{{ data.integrations_count }}</span></a>
						</nav>
					<# } #>
